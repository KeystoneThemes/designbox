...
class Table_Of_Contents extends Widget_Base {
	...
	public function get_name() {
		return 'wdb--table-of-contents';
	}

	public function get_title() {
		return esc_html__( 'Table of Contents', 'wdb-addons-pro' );
	}

	public function get_icon() {
		return 'wdb eicon-table-of-contents';
	}
	...
	protected function register_controls() {
		$this->start_controls_section(
			'table_of_contents',
			[
				'label' => esc_html__( 'Table of Contents', 'wdb-addons-pro' ),
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'label_block' => true,
				'default'     => esc_html__( 'Table of Contents', 'wdb-addons-pro' ),
			]
		);
		...
		$this->start_controls_tab( 'include',
			[
				'label' => esc_html__( 'Include', 'wdb-addons-pro' ),
			]
		);
		...
		$this->add_control(
			'headings_by_tags',
			[
				'label'              => esc_html__( 'Anchors By Tags', 'wdb-addons-pro' ),
		...
		$this->add_control(
			'container',
			[
				'label'              => esc_html__( 'Container', 'wdb-addons-pro' ),
				'type'               => Controls_Manager::TEXT,
				'ai'                 => [
					'active' => false,
				],
				'label_block'        => true,
				'description'        => esc_html__( 'This control confines the Table of Contents to heading elements under a specific container', 'wdb-addons-pro' ),
				'frontend_available' => true,
			]
		);
		...
		$this->start_controls_tab( 'exclude',
			[
				'label' => esc_html__( 'Exclude', 'wdb-addons-pro' ),
			]
		);
		...
		$this->add_control(
			...
			[
				'label'              => esc_html__( 'Anchors By Selector', 'wdb-addons-pro' ),
				'type'               => Controls_Manager::TEXT,
				'description'        => esc_html__( 'CSS selectors, in a comma-separated list', 'wdb-addons-pro' ),
				'default'            => [],
				'label_block'        => true,
				'frontend_available' => true,
			]
		...
		$this->add_control(
			'marker_view',
			[
				'label'              => esc_html__( 'Marker View', 'wdb-addons-pro' ),
				'type'               => Controls_Manager::SELECT,
				'default'            => 'numbers',
				'options'            => [
					'numbers' => esc_html__( 'Numbers', 'wdb-addons-pro' ),
		...
		$this->start_controls_section(
			'additional_options',
			[
				'label' => esc_html__( 'Additional Options', 'wdb-addons-pro' ),
			]
		);
		...
		$this->add_control(
			'word_wrap',
			[
				'label'        => esc_html__( 'Word Wrap', 'wdb-addons-pro' ),
		...
		$this->add_control(
			'minimize_box',
			[
				'label'              => esc_html__( 'Minimize Box', 'wdb-addons-pro' ),
				'type'               => Controls_Manager::SWITCHER,
				'default'            => 'yes',
				'frontend_available' => true,
			]
		...
		$this->add_control(
			'collapse_icon',
			[
				'label'       => esc_html__( 'Minimize Icon', 'wdb-addons-pro' ),
				'type'        => Controls_Manager::ICONS,
				'default'     => [
		...
		foreach ( $breakpoints as $breakpoint_key => $breakpoint ) {
			...
			/* translators: 1: Breakpoint label, 2: `<` character, 3: Breakpoint value. */
				esc_html__( '%1$s (%2$s %3$dpx)', 'wdb-addons-pro' ),
		...
		$minimized_on_options['desktop'] = esc_html__( 'Desktop (or smaller)', 'wdb-addons-pro' );

		$this->add_control(
			'minimized_on',
			[
				'label'              => esc_html__( 'Minimized On', 'wdb-addons-pro' ),
				'type'               => Controls_Manager::SELECT,
				'default'            => 'tablet',
		...
		$this->add_control(
			'hierarchical_view',
			[
				'label'              => esc_html__( 'Hierarchical View', 'wdb-addons-pro' ),
				'type'               => Controls_Manager::SWITCHER,
				'default'            => 'yes',
				'frontend_available' => true,
			]
		...
		$this->add_control(
			'collapse_subitems',
			[
				'label'              => esc_html__( 'Collapse Subitems', 'wdb-addons-pro' ),
				'type'               => Controls_Manager::SWITCHER,
				'description'        => esc_html__( 'The "Collapse" option should only be used if the Table of Contents is made sticky', 'wdb-addons-pro' ),
				'condition'          => [
		...
		$this->start_controls_section(
			'box_style',
			[
				'label' => esc_html__( 'Box', 'wdb-addons-pro' ),
		...
		$this->add_control(
			'loader_color',
			[
				'label'     => esc_html__( 'Loader Color', 'wdb-addons-pro' ),
				'type'      => Controls_Manager::COLOR,
				'selectors' => [
		...
		$this->add_responsive_control(
			'min_height',
			[
				'label'              => esc_html__( 'Min Height', 'wdb-addons-pro' ),
				'type'               => Controls_Manager::SLIDER,
		...
		$this->add_control(
			...
			[
				'label'      => esc_html__( 'Separator Width', 'wdb-addons-pro' ),
				'type'       => Controls_Manager::SLIDER,
				'size_units' => [ 'px', 'em', 'rem', 'custom' ],
				'selectors'  => [
		...
		$this->start_controls_section(
			'list_style',
			[
				'label' => esc_html__( 'List', 'wdb-addons-pro' ),
				'tab'   => Controls_Manager::TAB_STYLE,
			]
		);
		...
		$this->add_responsive_control(
			'max_height',
			[
				'label'      => esc_html__( 'Max Height', 'wdb-addons-pro' ),
				'type'       => Controls_Manager::SLIDER,
				'size_units' => [ 'px', 'em', 'rem', 'vh', 'custom' ],
				'range'      => [
					'px' => [
		...
		$this->add_control(
			'list_indent',
			[
				'label'      => esc_html__( 'Indent', 'wdb-addons-pro' ),
				'type'       => Controls_Manager::SLIDER,
				'size_units' => [ 'px', 'em', 'rem', 'custom' ],
				'default'    => [
					'unit' => 'em',
		...
		$this->add_control(
			...
			[
				'label'     => esc_html__( 'Underline', 'wdb-addons-pro' ),
				'type'      => Controls_Manager::SWITCHER,
				'selectors' => [
		...
		$this->start_controls_tab( 'active',
			[
				'label' => esc_html__( 'Active', 'wdb-addons-pro' ),
			]
		);
		...
		$this->add_control(
			'heading_marker',
			[
				'label'     => esc_html__( 'Marker', 'wdb-addons-pro' ),
		...
		if ( 'yes' === $settings['minimize_box'] ) {
			$this->add_render_attribute(
				...
				[
					...
					'role'       => 'button',
					'tabindex'   => '0',
					'aria-label' => esc_html__( 'Open table of contents', 'wdb-addons-pro' ),
				]
			);
			$this->add_render_attribute(
				'collapse-button',
				[
					...
					'role'       => 'button',
					'tabindex'   => '0',
					'aria-label' => esc_html__( 'Close table of contents', 'wdb-addons-pro' ),
				]
			);
		}
