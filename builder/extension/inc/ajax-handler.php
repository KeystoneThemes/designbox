...
class Ajax_Handler {
	...
	public static function init() {
		...
		add_action( 'wp_ajax_wdb_mailchimp_ajax', [ __CLASS__, 'mailchimp_prepare_ajax' ] );
		add_action( 'wp_ajax_nopriv_wdb_mailchimp_ajax', [ __CLASS__, 'mailchimp_prepare_ajax' ] );

		//popup action
		add_action( 'wp_ajax_wdb_load_popup_content', [ __CLASS__, 'wdb__popup_content' ] );
		add_action( 'wp_ajax_nopriv_wdb_load_popup_content', [ __CLASS__, 'wdb__popup_content' ] );
	}
	...
	public static function mailchimp_lists() {
		if ( ! wp_verify_nonce( $_REQUEST['nonce'], 'wdb-addons-editor' ) ) {
			exit( 'No naughty business please' );
		}

		$api = ! empty( $_REQUEST['api'] ) ? $_REQUEST['api'] : '';

		if ( ! class_exists( 'WDBAddonsEX\Widgets\Mailchimp\Mailchimp_Api' ) ) {
			include_once WDB_ADDONS_EX_PATH . 'widgets/mailchimp/mailchimp-api.php';
		}
	...
	public static function mailchimp_prepare_ajax() {
		if ( ! wp_verify_nonce( $_REQUEST['nonce'], 'wdb-addons-frontend' ) ) {
			exit( 'No naughty business please' );
		}
	...
	/**
	 * wdb popup content Ajax call
	 */
	public static function wdb__popup_content() {
		if ( ! wp_verify_nonce( $_REQUEST['nonce'], 'wdb-addons-frontend' ) ) {
			exit( 'No naughty business please' );
		}
		...
		$settings = wdb_addons_get_widget_settings($post_id, $widget_id);
