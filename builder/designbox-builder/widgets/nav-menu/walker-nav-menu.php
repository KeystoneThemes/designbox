...
namespace WDB_ADDONS\Widgets\Nav_Menu;

use WDB_ADDONS\WDB_Theme_Builder;

defined( 'ABSPATH' ) || die();

class WDB_Menu_Walker extends \Walker_Nav_Menu {
	...
	public function is_megamenu_enable( $menu_slug ) {
		...
		$return   = false;
		$options  = get_option( "wdb_menu_options_" . $menu_id );
	...
	public function start_el( &$output, $data_object, $depth = 0, $args = null, $current_object_id = 0 ) {
		...
		$mega_item_settings = get_post_meta( $menu_item->ID, 'wdb_mega_menu_settings', true );
		...
		if ( $this->is_megamenu_enable( $args->menu ) && isset( $mega_item_settings['menu-item-template'] ) && ! empty( $mega_item_settings['menu-item-template'] ) ) {
			$classes[]      = 'wdb-mega-menu';
			$buildercontent = WDB_Theme_Builder::$_instance->render_build_content( $mega_item_settings['menu-item-template'] );
		...
		if ( $this->is_megamenu_enable( $args->menu ) && isset( $mega_item_settings['menu-item-badgetext'] ) && ! empty( $mega_item_settings['menu-item-badgetext'] ) ) {
			...
			$badge = '<span class="wdb-menu-badge" style="' . $badge_style . '">' . $mega_item_settings['menu-item-badgetext'] . '</span>';
		}
		...
		if ( in_array( 'menu-item-has-children', $classes ) || ! empty( $buildercontent ) ) {
			...
			if ( ! empty( $args->submenu_indicator_icon ) ) {
				$submenu_indicator .= '<span class="wdb-submenu-indicator">' . $args->submenu_indicator_icon . '</span>';
			}
		}
		...
		if ( ! empty( $buildercontent ) ) {
			$item_output .= sprintf( '<div class="wdb-mega-menu-panel" style="%1s">%2s</div>', $styles, $buildercontent );
		}
